...
#select pessoa.nome, endereco.rua, endereco.cidade from pessoa right join endereco on pessoa.id = endereco.pessoa_id;

select pessoa.nome, endereco.rua, endereco.cidade, telefone.numero from pessoa left join endereco on pessoa.id = endereco.pessoa_id left join telefone on pessoa.id = telefone.pessoa_id;


######Exercício######

#A) Suponha que você tem duas tabelas: "Clientes" e "Pedidos". Crie uma consulta que liste todos os pedidos juntamente com o nome do cliente que fez cada pedido usando um INNER JOIN;
select clientes.nome, pedidos.id from clientes inner join pedidos on clientes.id = pedidos.clientes_id;

#B) Você possui duas tabelas: "Departamentos" e "Funcionários". Alguns funcionários não estão atribuídos a nenhum departamento. Crie uma consulta que liste todos os funcionários e seus respectivos departamentos, incluindo os funcionários que não estão atribuídos a nenhum departamento, usando um LEFT JOIN;
#